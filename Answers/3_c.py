...
trainData_rescaled = (trainData - trainData.min(axis=0)) / (trainData.max(axis=0) - trainData.min(axis=0)) * 10

n_sample = trainData_rescaled.shape[0]
max_nCluster = 8
...
for k in range(max_nCluster):
    nCluster = k + 1
    member, centroid, wc_distance = KMeansCluster(trainData_rescaled, nCluster, nTrial = 20, randomSeed = 20231225)
...
result_df = pandas.DataFrame({'N Cluster': nClusters,
                              ...
                              'Elbow': Elbow,})
result_df.to_csv('3_coutput.csv')
